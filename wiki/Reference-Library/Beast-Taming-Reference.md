---
title: "Beast Taming Reference"
order: 2
published: true
draft: false
---
# Beast Taming Reference

Parent: [Reference Library Index](/mod/alecs-animal-husbandry/reference-library) | [Home](/mod/alecs-animal-husbandry/readme)
Related hub: [Items Index](/mod/alecs-animal-husbandry/items)

This page is a practical quick-reference for Beast taming.

![Beast Taming Gear](https://wiki.hytalemodding.dev/storage/mods/019ce94e-972f-717d-8881-7a3d542de0c3/files/d7b61c44-e95e-4e07-85d9-8f984b82784c.png)

## Required Items
1. [Tranquilizer Potion](/mod/alecs-animal-husbandry/items/tranquilizer-potion)
2. [Tranquilizer Arrows](/mod/alecs-animal-husbandry/items/tranquilizer-arrows)
3. [Tranquilizer Shortbow](/mod/alecs-animal-husbandry/items/tranquilizer-shortbow)
4. Preferred food ([see table below](#beast-taming-and-feeding-table)).
5. (Recommended) [Beast Command Flute](/mod/alecs-animal-husbandry/items/beast-command-flute) for post-tame command control.

[![Beast Taming Demo Video](https://wiki.hytalemodding.dev/storage/mods/019ce94e-972f-717d-8881-7a3d542de0c3/files/49863e5d-433c-4bc3-8de2-b79649f62499.png)](https://www.youtube.com/watch?v=Yo5Aemwwjfo)

## Beast Taming Flow
1. Brew [Tranquilizer Potion](/mod/alecs-animal-husbandry/items/tranquilizer-potion).
2. Craft [Tranquilizer Arrows](/mod/alecs-animal-husbandry/items/tranquilizer-arrows).
3. Craft [Tranquilizer Shortbow](/mod/alecs-animal-husbandry/items/tranquilizer-shortbow).
4. Use tranquilizer shots while managing the Beast's health carefully.
5. Tranquilizer requires the target to be at **20% HP or lower** to take effect.
    - It doesn't matter if you build the tranquilizer stacks or get it below 20% first.
6. Once the Beast is asleep, you can feed it it's preferred food to tame it.
7. You can press `F` on a tamed Beast to quick-toggle simple modes (`Follow`, `Hold`, `Idle`).
8. Left-click with your [Beast Command Flute](/mod/alecs-animal-husbandry/items/beast-command-flute) to link it for more advanced control.

## Tranquilizer Shot Math
- Tranquilizer duration per hit: **30 seconds**
- Hits stack duration
- Shots needed: **ceiling(tranquilizer threshold / 30)**
- Values below assume steady hit pressure without long gaps

## Beast Taming and Feeding Table
| Beast | Preferred Food | Threshold (seconds) | Shots Needed |
|---|---|---:|---:|
| Larva Silk | Egg | 20 | 1 |
| Rat | Raw Pork | 20 | 1 |
| Fox | Raw Chicken | 50 | 2 |
| Molerat | Egg | 50 | 2 |
| Slug Magma | Raw Wildmeat | 50 | 2 |
| Snake Cobra | Egg | 50 | 2 |
| Snake Marsh | Egg | 50 | 2 |
| Snake Rattle | Egg | 50 | 2 |
| Spider | Raw Chicken | 50 | 2 |
| Spider Cave | Raw Chicken | 50 | 2 |
| Fen Stalker | Raw Fish | 80 | 3 |
| Hyena | Raw Wildmeat | 80 | 3 |
| Leopard Snow | Raw Wildmeat | 80 | 3 |
| Raptor Cave | Raw Chicken | 80 | 3 |
| Scorpion | Raw Chicken | 80 | 3 |
| Wolf Black | Raw Wildmeat | 80 | 3 |
| Wolf White | Raw Wildmeat | 80 | 3 |
| Bear Grizzly | Raw Fish | 110 | 4 |
| Bear Polar | Raw Fish | 110 | 4 |
| Crocodile | Raw Fish | 110 | 4 |
| Emberwulf | Raw Wildmeat | 110 | 4 |
| Snapdragon | Raw Beef | 110 | 4 |
| Tiger Sabertooth | Raw Wildmeat | 110 | 4 |
| Toad Rhino | Raw Fish | 110 | 4 |
| Toad Rhino Magma | Raw Beef | 110 | 4 |
| Yeti | Raw Beef | 140 | 5 |
| Rex Cave | Raw Beef | 170 | 6 |

> [Screenshot Placeholder: Beast entering tranquilized sleep]
> [Screenshot Placeholder: Taming interaction after tranquilized sleep]


